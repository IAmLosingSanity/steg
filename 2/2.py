...
for lam in lam_values:
    # Встраивание
    stego_image = embed_watermark(original_image, watermark, lam)

    # Преобразуем в uint8 для сохранения
    stego_uint8 = np.clip(stego_image, 0, 255).astype(np.uint8)
    stego_pil = Image.fromarray(stego_uint8)

    # Сохраняем с именем, отражающим значение λ
    filename = f"stego_lambda_{lam:.2f}.png"
    filepath = os.path.join(output_dir, filename)
    stego_pil.save(filepath)
